...
Install the bundled plugin directly from GitHub:

```sh
amp plugins add --auto-update https://raw.githubusercontent.com/nguyenphutrong/amp-plugins/main/plugins/markdown-commands/dist/index.ts
```

Reload plugins from Amp's command palette with `Ctrl+O`, then select `plugins: reload`. Amp checks the source URL for updates when `--auto-update` is enabled.
...
From the repository root, install dependencies and run this workspace's tests:

```sh
bun install
bun test plugins/markdown-commands
```

Build the single-file installation artifact:
